...
docker run -d -p ${mapedport}:${CUSTOM_MANAGED_SERVER_PORT} -v ${scriptDir}/properties:/u01/oracle/properties ${ENV_ARG} --volumes-from ${adminhost} --link ${adminhost}:${adminhost} --name ${managedname} etxnode/weblogic12213-domain-in-volume /u01/oracle/container-scripts/startManagedServer.sh
