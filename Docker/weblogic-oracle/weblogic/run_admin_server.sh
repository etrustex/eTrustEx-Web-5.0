...
echo "docker run -d -p 9002:9002 -p 7001:7001 -p 5006:8453 --name ${adminhost} --hostname ${adminhost} -v ${scriptDir}/properties:/u01/oracle/properties -v ${domainhostvol}:/u01/oracle/user_projects/domains/base_domain ${ENV_ARG} etxnode/weblogic12213-domain-in-volume"

docker run -d -p 9002:7001 -p 7001:7001 -p 5006:8453 --name ${adminhost} --hostname ${adminhost} -v ${scriptDir}/properties:/u01/oracle/properties -v ${domainhostvol}:/u01/oracle/user_projects/domains/base_domain ${ENV_ARG} etxnode/weblogic12213-domain-in-volume
