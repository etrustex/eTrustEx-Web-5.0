      ...
      >
        <div class="d-flex flex-nowrap align-items-center">
          <span class="ico-conjunction">
            <i v-if="isSortable && field.sortable" class="ico-sort-default me-0" :class="{'ico-sort-default': sortBy !== field.key,
               [`ico-sort-${sortDescBoolean ? 'descending' : 'ascending'}`]: field.key === sortBy}"></i>
          </span>
          <div>
            <slot
...
/*
 * Taken from https://github.com/bootstrap-vue/bootstrap-vue-next/tree/main/packages/bootstrap-vue-next/src/components/BTable
 * while bootstrap-vue is not available for vue 3
 */
import { computed, onMounted, ref, toRef, useSlots, watch } from 'vue'
import VerticalAlign from '@/shared/views/bootstrap_vue/types/VerticalAlign'
...
import { BTableProvider, BTableSortCompare } from '@/shared/views/bootstrap_vue/types/BTable/BTable'
import useItemHelper from '@/shared/views/bootstrap_vue/utils/itemHelper'
import useBooleanish from '@/shared/views/bootstrap_vue/composables/useBooleanish'
import { cloneDeepAsync } from '@/shared/views/bootstrap_vue/utils/object'
import { titleCase } from '@/shared/views/bootstrap_vue/utils/stringUtils'
import BTableSimple from '@/shared/views/bootstrap_vue/BTtable/BTableSimple.vue'
import { filterEvent } from '@/shared/views/bootstrap_vue/helpers/filter-event'
import BSpinner from '@/shared/views/bootstrap_vue/BTtable/BSpinner.vue'
...
interface BTableEmits {
  ...
  (e: 'sort-changed', ...value: Parameters<(sortBy: string, isDesc: boolean) => any>): void
...
const sortDescBoolean = ref(props.sortDesc)
...
const handleFieldSorting = (field: TableField) => {
  ...
  if (isSortable.value && fieldSortable === true) {
    sortDescBoolean.value = !sortDescBoolean.value
    if (fieldKey !== props.sortBy) {
      sortBy.value = fieldKey
    }
    emit('sort-changed', fieldKey, sortDescBoolean.value)
  }
}
...
const callItemsProvider = async () => {
  ...
  if (response instanceof Promise) {
    try {
      ...
      return await itemHelper.updateInternalItems(items)
    } finally {
  ...
  try {
    return await itemHelper.updateInternalItems(response)
  } finally {
    if (internalBusyFlag.value) {
      internalBusyFlag.value = false
    }
